...
function PaymentForm({ token, onPaymentResult }) {
  ...
  return (
    <section className="card">
      <div className="section-header">
        <div>
          <div className="section-eyebrow"><FiSend className="icon" />Payment</div>
          <h3>Make a payment</h3>
          <p className="card-subtitle">
            Send money from your package account. Fraud detection will automatically check the transaction.
          </p>
        </div>
      </div>
      ...
      {result && (
        <div className={result.success ? 'success' : 'error'} style={{ marginTop: '18px' }}>
          <strong>{result.success ? (result.suspicious ? 'Suspicious' : 'Success') : 'Failed'}</strong>
          <p style={{ margin: '6px 0 0' }}>{result.message}</p>
          {result.suspicious && result.fraudReason && (
            <p style={{ margin: '6px 0 0', fontWeight: 600 }}>Reason: {result.fraudReason}</p>
          )}
